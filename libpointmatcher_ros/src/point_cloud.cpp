...
namespace PointMatcher_ros
{
    ...
    std::vector<double> intensities(rosMsg.intensities.size());
		...
		for (size_t i = 0; i < rosMsg.ranges.size(); ++i)
		{
            ...
            if (range >= rosMsg.range_min && range < rosMsg.range_max)  // FUCK using UTS 2D laser data, < & <=
      {
...
//      cout<<"x:  "<<x<<"  y:  "<<y<<"  a:  "<<angle<<"  r:  "<<range<<endl;  // have problem here, 07.23
...
//        cout<<"x:  "<<p_out.point.x<<"  y:  "<<p_out.point.y<<endl;
        if(force3D)
