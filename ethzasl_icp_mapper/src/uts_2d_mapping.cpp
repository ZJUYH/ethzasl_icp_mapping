...
//    cout<<"T odom to scannner:\n"<<TOdomToScanner<<endl;
 	...
 	if(!icp.hasMap())
    {
        ...
        if(loopCnt % saveFreq == 0)
        {
            if(savePose.is_open())
            {
                // save 3*3 matrix for 2d mapping
                savePose << saveCnt << "  " << saveCnt << "  "
                            << TOdomToMap(0,0) << "  " << TOdomToMap(0,1) << "  " << TOdomToMap(0,2) << "  "
                               << TOdomToMap(1,0) << "  " << TOdomToMap(1,1) << "  " << TOdomToMap(1,2) << "  "
                                  << TOdomToMap(2,0) << "  " << TOdomToMap(2,1) << "  " << TOdomToMap(2,2) << endl;
//                savePose.flush();
